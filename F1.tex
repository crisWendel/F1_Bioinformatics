...
\subsubsection{How to add, commit and push}
\texttt{\$ git init} ~ initializes a new Git repository. \\
\texttt{\$ git config} ~ configures Git for the first time.\\ 
\texttt{\$ git help} ~ shows common git commands. \\
\texttt{\$ git status} ~ checks the status of the repo. See which files are inside it, which changes still need to be committed, which branch of the repo I am currently working on. \\
\texttt{\$ git add} ~ brings new files to Git's attention. After adding a file, it is included in Git's "snapshots" of the repo.\\
\texttt{git commit -m} ~ is the most important command. It takes a "snapshot" of the repo. The command \texttt{git commit -m } usually is followed by "description", meaning what changes were made. \\
\texttt{git push} ~ this makes my commits visible online on Github. I "push" the changes up to GitHub. \\
